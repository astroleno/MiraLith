...
| `cp0.2-v3-source-direction-bridge-manifest.json` | Current v3 media hashes, real negative CoScroll yaw contract, constant-phase proof (no reversal), blue-particle constraints, and A/B/C linear & scrub identity. |
| `cp0.2-v2-continuous-blue-bridge-manifest.json` | Historical v2 identity. It is superseded because it normalized the real negative source yaw to positive screen rotation. |

`cp0.2-bridge-variants-manifest.json` and its native scroll trace describe the historical `cp02-v1` review only; v1 was author-NO-GO because the solid character read as stopping before particleization and its particles warmed too soon. `cp02-v2` is also historical because it reversed the live source sign. The current referenced review media remain local-only under `apps/site/.generated/post-coscroll-editorial/cp02-v3/`. The durable current comparison rationale is [CP0.2 v3 bridge variants](../cp0.2-bridge-variants-v3.md).
