...
The renderer exposes the combined contract as profile
`hierarchical-quadratic-quartic-v1`. Any future profile or role-assignment change must include
numerical reference tests, a hierarchy-identity hash change, and visual A/B.
...
The Phase -1 renderer evaluates the same quadratic/quartic profile analytically from each visible
...
The experiment rasterizes the already-locked Earth-local V3-derived `base → tower → detail`
hierarchy into one transient local RGBA density field inside the real aggregate carrier. Its alpha
is density and its RGB stores the precomputed local density gradient, so cloud-top/side lighting
follows the same mass boundary without an extra runtime texture lookup. The field is
generated from that hierarchy only: no hand-authored noise, synthetic patch, or new cloud asset is
allowed. The carrier has only glue weight in the field; base, tower, and detail retain their
separate V3-derived masses. The same proxy ellipsoid, Earth hit clipping, source/hash contract,
and view-depth boundary from Phase -1.1 define the occupied domain.
