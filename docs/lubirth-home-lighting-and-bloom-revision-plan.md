...
- 云层继续保持“一张 packed 纹理 + 一层云壳”：R 为覆盖度、GB 为切线法线、A 为厚度；A 参与最高 `0.0045R` 的径向顶点置换，fragment 只增加一次视向 relief sample，并继续复用现有太阳偏移自遮蔽和地表云影。此处记录实现状态，视觉验收闭环见 11.7。
...
### 11.7 云体积视觉验收补记（2026-07-14，已被 11.9 取代）

- 旧实现虽然具备 A 通道置换与一次 relief sample，但 lite shader 在切线处把所有云统一衰减到透明，厚云最能表现高度的外轮廓仍不可读；仅检查 `heightScale` 与 `reliefSamples` 也无法证明像素结果。
- 当时的 lite 云壳改为厚度与覆盖度感知的 limb fade：薄云继续自然消失，厚云在地平线保留受控的冷灰轮廓；`HOME_LITE_CLOUD_SHELL` 的 `limbFadeStart` / `limbFadeEnd` 开始真正参与 shader。后续 1× 复核确认该冷灰轮廓仍会读成第二层球壳，现已由 11.9 的自然位移轮廓取代。
- 同一次 relief sample 的厚度与覆盖度差值同时驱动顶部增亮和背光侧压暗；纹理采样数、云壳数、DPR 和后处理均未增加。
- 新增固定 `1440×960` 近景像素回归，对比 `cloud=surface` 与 `cloud=shell-lite`。该对比只负责验证厚云越过基础云壳的列占比、强轮廓列占比和轮廓颜色差；它不能单独证明 relief 顶面/侧面光照生效。
- 当时固定样本结果：位移轮廓列占比约 `0.43`、强轮廓列占比约 `0.39`、位移带平均颜色差约 `2.41`；这些数据只保留为历史问题定位记录，不再代表当前视觉闭环，现行回归与状态见 11.9。

### 11.8 Relief 光照与背光轮廓回归补强（2026-07-15，已被 11.9 取代）

- lite shader 新增 `reliefLightingStrength` uniform。它在生产固定为 `1`，只有 `visualTest=pixels&cloudReliefLighting=off` 能关闭新加入的 `reliefTopLight` / `reliefSideShade`，几何置换、relief 采样、法线、自遮蔽、alpha 和地表云保持完全一致。
- relief 回归改为同一正面光近景下比较 uniform `0 → 1`，直接要求顶面正增量、侧面负增量、有效像素占比与差值离散度。固定样本为：有效像素占比约 `0.120`、顶面/侧面有效像素 `98/95`、两侧 p90 差值约 `4.72/5.79`、差值标准差约 `1.02`；删除任一 relief 光照分支都会使对应断言失败。
- 新增真实背光样本 `2026-07-12T00:00:00Z`，并断言世界空间光线 Z ≤ `-0.75`。背光厚云仍须产生可见轮廓，同时限制轮廓亮度 p95 ≤ `48/255`、相对背景的正向亮度增量 p95 ≤ `47/255`、最大亮度 ≤ `64/255`。
- 为满足该上限，夜侧冷灰轮廓由固定高亮改为受 `silhouetteLight` 控制的颜色与混合权重；固定背光结果为 p95 `46.00/255`、正向增量 p95 `45.14/255`、最大亮度 `61.00/255`，不形成灰白描边。
- 事后 1× 复核发现该方案仍未形成可信云体：`max(base, relief)` 造成膨胀双边，relief 明暗仅集中在地平线窄带，`silhouetteColor` 仍会读成第二层球壳。因此 11.8 只保留为问题定位记录，不再代表当前实现或视觉闭环。

### 11.9 单步视差云体候选（2026-07-15，已被 11.10 取代）

- 保持一张 packed 云场、一层云壳和原有采样预算。A 通道仍驱动最高 `0.0045R` 顶点置换，GB 仍作为切线法线；fragment 仍只读取 base、view-relief 和既有 sun-occlusion 三个样本。
- 删除 `max(baseCloudField, reliefField)` 并集，改为按视角与厚度在 base/relief 间执行单步高权重选择混合。视向偏移在云主体保持非零，只在掠射角增强，避免同时显示原位置与偏移位置形成的膨胀、拖影和双边。
- 顶面/侧面光照改由已存在的 `occlusionField` 与当前视差样本形成太阳方向厚度梯度；不再使用 `grazingView` 门控，因此正面可见的主体云团也获得方向明确的顶面增亮与侧面压暗。
- 完全删除 `silhouetteRetention`、`silhouetteColor` 和 fragment 颜色下限。地平线只保留由覆盖度、厚度、真实顶点位移和厚度感知 alpha 共同产生的轮廓；未受光的切线云额外衰减，避免遮住星空形成黑色第二壳。
- relief 回归迁移到地球主体内部 ROI，而非地平线向内 `2–20px` 窄带。固定 ROI 采样 `79,998` 个像素：整块 ROI 平均绝对亮度差 `2.15/255`、p95 `15/255`、最大差 `77/255`、有效像素占比约 `10.49%`；顶面/侧面有效像素分别为 `5,481/2,909`，1× 裁切图中可直接辨认。
- 背光样本不再依赖人工冷灰描边：世界光线 Z 为 `-0.858` 时，轮廓亮度 p95 `39.57/255`、正向增量 p95 `38.72/255`、最大值 `51.78/255`，均低于既有 `48/47/64` 上限。
- 当前状态为“技术回归通过、等待 1× 视觉确认”，不得仅凭测试绿色标记云体视觉闭环。

### 11.10 数据安全的多尺度云体候选（2026-07-15）

- packed 云场由有损 `1536×768` WebP 改为 `1024×512` 无损 RGBA PNG，仍是一张纹理和一次 GPU 上传。生成文件为 `947,161 bytes`，`ffprobe` 确认为 `rgba`，生成脚本会重新解码并要求 R/G/B/A 四通道最大误差全部为 `0`；不再经过 YUV 转换或色度抽样。
- R 继续保存覆盖度，GB 保存切线法线，A 保存厚度/高度。高度由 detail、shape、body、weather 四级场合成；法线同时读取 `2/8/24px` 三档高度梯度，因此云团内部具有宽尺度丘状变化，不再只沿覆盖度边缘产生差值。
- 太阳采样向量改为受限的 `tangent / normal` 比值，采样距离随入射角从 `0.0012` 扩展到 `0.0068 UV`，并继续乘以局部厚度。低角度光因此获得更长的自遮蔽路径，而不是与正午共用固定描边宽度。
- relief 光照不再使用 `abs(current - offset)` 边缘门控；顶面和侧面来自多尺度 GB 法线相对基础球面法线的差，自遮蔽来自入射角缩放后的厚度场。低覆盖度边缘额外降低颜色与 alpha，背光时不再形成白丝带或黑色薄脊。
- 回归新增测试专用 core/edge mask。固定 `1440×960` 云主体 ROI 共采样 `125,280` 个像素，其中云核心 `6,224`、云边缘约 `17,283`；核心 relief 有效像素占比约 `74.7%`、平均绝对差 `4.23/255`、p90 `11/255`，边缘/核心平均差比约 `0.034`，变化明确集中在云体内部而非轮廓。
- 背光固定样本世界光线 Z 为 `-0.858`。地平线外侧不再产生可测第二壳；云边缘相对 surface 的亮度增量 p95 `13/255`、p99 `19/255`、最大值 `39/255`，低于 `32/48/64` 上限。
- 近、中、远景在同一页面内切换，投影半径依次为 `1028.88 / 475.25 / 170.66px`；三帧保持同一 texture UUID、单云壳和固定 offset，没有因路由重载或双纹理造成跳变。
- 月球同时提高直射曝光、夜面底色和 earthshine，但仍保留月相与地形对比，解决整帧中月球近黑的问题。
- 当前状态仍为“技术回归通过、等待用户 1× 视觉确认”。在确认前不得提交，也不得把首页云体标记为视觉闭环。
