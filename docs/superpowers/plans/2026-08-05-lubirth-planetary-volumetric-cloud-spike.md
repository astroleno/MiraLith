...
> 2026-08-12 cloud-scale similarity supersession：`docs/superpowers/specs/2026-08-12-lubirth-takram-cloud-scale-similarity-design.md` 与对应实施计划已经取代独立 `260/40 km` 搜索及 sample-budget causal A/B。commit `589e051` 上的首个 12 帧 stock matrix 使用统一 `PUBLIC_PARAMETER_SIMILARITY` resolver，但仍保留未缩放的 `localWeatherRepeat=[100,100]`；correctness review 因此将它降级为 `UNSCALED_STOCK_WEATHER_CONTROL`，不能据此分类统一缩放或授权 mip。下一步 Stage A1 固定 stock local cube-sphere weather，并单独测试 `[100/S,100/S]`，同时从实际 runtime shader/uniform 回读 mip 状态并记录 installed shader/build/patch SHA-256。Stage B/C、V3 分类、Conditional Task M、sample-budget A/B、Task 0P 与原 Task 0–8 均未运行且继续锁定。历史证据见 `docs/lubirth-planetary-cloud-evidence/2026-08-12/takram-cloud-scale/`。
> 计划日期：2026-08-05。
