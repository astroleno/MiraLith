...
> 2026-08-09 scale/morphology amendment：Task 1–2 已在 commit `087563e` 上重建。review target 现在使用真实球面 arc，三个近景审计点位于画面中心，目标高度与地表距离均通过测试。纠正后的投影证明共享 repeat 无法同时覆盖三个近景：shape 共同区间为 `[0.0002132923, 0.0000827093]`，detail 为 `[0.0010238031, 0.0004411164]`，均为空。10 个物理范围候选也全部退化为近乎单一连通遮罩，没有 candidate 同时通过三个 near views。计算型 checkpoint 为 `HORIZONTAL_MORPHOLOGY_SCALE_FAIL`；Task 3–6、Task 0P 与原 Task 0–8 继续锁定，后续必须先以新 amendment 明确授权 near/aerial presentation LOD。详见 `docs/lubirth-planetary-cloud-evidence/2026-08-09/v3-morphology/`。
> 计划日期：2026-08-05。
