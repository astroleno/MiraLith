...
> 2026-08-12 cloud-scale similarity supersession：`docs/superpowers/specs/2026-08-12-lubirth-takram-cloud-scale-similarity-design.md` 与对应实施计划已经取代独立 `260/40 km` 搜索及 sample-budget causal A/B。首个 12 帧 stock matrix 因未缩放 `localWeatherRepeat=[100,100]` 被降级为 `UNSCALED_STOCK_WEATHER_CONTROL`。clean-HEAD Stage A1 随后完成 `[100/S,100/S]` health control，并从实际 runtime shader/uniform 验证原生单一 `rayDistance * 1e-5` 与 installed shader/build/patch SHA-256；三档都恢复了 stock weather signal，但均未达到轨道视觉门。用户授权的只读 Conditional Task M 随后以同一 instrumentation 在 exact native frames `16/32/48` 对比健康 `S=1` 和 `S=80/120/160`。所有人口有效；`S=160` 达到 premature-mip 门，但必需的 `S=120` 只有 `71.4794%` 样本达到 mip excess `>=1.0`，未通过冻结 `75%` 阈值，结论为 `MIP_CAUSAL_HYPOTHESIS_REJECTED`。mip patch 未授权、未实现；Stage B/C、V3 分类、sample-budget A/B、Task 0P 与原 Task 0–8 继续锁定。下一步若调整垂直形态、光学密度或大气合成，必须先写新计划。证据见 `docs/lubirth-planetary-cloud-evidence/2026-08-12/takram-cloud-scale/task-m-mip-diagnostic/`。
> 计划日期：2026-08-05。
