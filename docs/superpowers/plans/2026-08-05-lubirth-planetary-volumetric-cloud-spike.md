...
> 2026-08-09 scale/morphology amendment：最新 full/raw 证据已经把失败收敛为 `V3_ADAPTER_SIGNAL_VISIBLE_MORPHOLOGY_FAIL`；V3 footprint 可见，但仍读成贴地色块和颗粒噪点。独立的 [`LuBirth Takram V3 Scale and Morphology Implementation Plan`](./2026-08-09-lubirth-takram-v3-scale-and-morphology.md) 已执行到 Task 2：米制尺度审计与跨视角 horizontal candidate replay 均可复现，但没有共同 near-view candidate，当前 checkpoint 为 `HORIZONTAL_MORPHOLOGY_SCALE_FAIL`。该计划因此锁定 Task 3–6、Task 0P 与原 Task 0–8；在 camera/physical-scale contract amendment 前不得继续垂直、temporal、lighting 或成本调参。详见 `docs/lubirth-planetary-cloud-evidence/2026-08-09/v3-morphology/`。
> 计划日期：2026-08-05。
