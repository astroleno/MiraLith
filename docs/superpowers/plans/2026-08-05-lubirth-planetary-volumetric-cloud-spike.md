...
> 2026-08-09 opening-only Task 2O/2S supersession：当前产品 gate 不再由 `2.5 / 50 / 200 km` 近景决定；这些视角只保留为未来低空镜头诊断。commit `14a9d05` 在真实 opening progress `0.00 / 0.06 / 0.12 / 0.18` 上完成 shape `220 / 260 / 300 km` × detail `30 / 40 km` 的 144 张源帧、24 张 cloud-only mask 与 24 份原生 sample-count 缓冲。六组 shape 均达标，只有三组 `40 km` detail 达标；所有 final output 仍缺少云地分离、透明层次、受光/背光体积变化和稳定 BSM 响应。raw fragmentation 与 final/raw `7.58–10.49%` attenuation 仍是有效观察，但旧 primary `p50=2` 对 mask threshold 高度敏感，stage A/B 未锁定同一 temporal frame，且没有同相机 healthy control。commit `2839f2b` 已修复 capture protocol，权威 checkpoint 修正为 `OPENING_MORPHOLOGY_VISUAL_FAIL / STAGE_ISOLATION_INCONCLUSIVE_WITH_ATTENUATION_OBSERVED / SAMPLING_CAUSALITY_UNVERIFIED / ROOT_CAUSE_NOT_YET_ISOLATED`；Task 3–6、Task 0P 与原 Task 0–8 继续锁定，不得继续水平 repeat 搜索、提高 sample budget 或直接授权 density representation 重构。详见 `docs/lubirth-planetary-cloud-evidence/2026-08-09/v3-opening-morphology/`。
> 计划日期：2026-08-05。
