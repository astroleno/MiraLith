...
> 2026-08-09 scale/morphology amendment：Task 1–2 已在 commit `1063909` 上重建。四个视角现在观察同一个固定 V3 spherical UV，三个近景相机从目标反向沿真实球面 arc 求解，审计点位于画面中心。纠正后的 east/north 双轴投影证明共享 scalar repeat 无法覆盖三个近景：shape 共同区间为 `[0.0003008209, 0.0000282150]`，detail 为 `[0.0014439402, 0.0001504800]`，均为空；`axisEligibleCandidates=[]`。最大连通面积已降为诊断项，不再被用来否决内部 billow；raw-off、cloud-off 与 native frame 1 temporal capture 已完整持久化。计算型 checkpoint 仍为 `HORIZONTAL_MORPHOLOGY_SCALE_FAIL`；Task 3–6、Task 0P 与原 Task 0–8 继续锁定。后续必须先以新 amendment 修改 view-space acceptance contract，或明确授权 anisotropic ENU morphology representation；near/aerial scalar LOD 本身不足以修复单一视角内的双轴跨度。详见 `docs/lubirth-planetary-cloud-evidence/2026-08-09/v3-morphology/`。
> 计划日期：2026-08-05。
