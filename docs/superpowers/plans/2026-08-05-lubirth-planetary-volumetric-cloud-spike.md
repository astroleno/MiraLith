...
> 2026-08-09 scale/morphology amendment：最新 full/raw 证据已经把失败收敛为 `V3_ADAPTER_SIGNAL_VISIBLE_MORPHOLOGY_FAIL`；V3 footprint 可见，但仍读成贴地色块和颗粒噪点。下一步不再在本大计划中追加散乱调参，改由独立的 [`LuBirth Takram V3 Scale and Morphology Implementation Plan`](./2026-08-09-lubirth-takram-v3-scale-and-morphology.md) 执行。该计划通过前 Task 0P 与原 Task 0–8 继续锁定。
> 计划日期：2026-08-05。
