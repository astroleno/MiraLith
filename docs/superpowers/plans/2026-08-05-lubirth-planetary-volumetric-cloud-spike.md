...
> 2026-08-12 cloud-scale similarity supersession：`docs/superpowers/specs/2026-08-12-lubirth-takram-cloud-scale-similarity-design.md` 与对应实施计划已经取代独立 `260/40 km` 搜索及 sample-budget causal A/B。commit `589e051` 上的 clean-HEAD Stage A 使用统一 `PUBLIC_PARAMETER_SIMILARITY` resolver，完整同步 shape/detail、四层高度/密度、turbulence、Clouds/Shadow 步长/射线与双 `minExtinction`，并在 stock/V3 共用显式官方 R/G/B/A（空 A）合同下通过 `12` 组 runtime parity 回归。stock coverage `0.3` 的 `S=80/120/160 × progress 0.00/0.06/0.12/0.18` 最终帧均没有可读 stock 云体，因此 checkpoint 为 `PUBLIC_PARAMETER_SIMILARITY_VISUAL_FAIL_MIP_UNPROVEN`，`stockPassingScales=[]`。Stage B/C、V3 分类、mip patch、sample-budget A/B、Task 0P 与原 Task 0–8 均未运行且继续锁定。证据见 `docs/lubirth-planetary-cloud-evidence/2026-08-12/takram-cloud-scale/`。
> 计划日期：2026-08-05。
