...
> 2026-08-12 cloud-scale similarity supersession：`docs/superpowers/specs/2026-08-12-lubirth-takram-cloud-scale-similarity-design.md` 与对应实施计划已经取代独立 `260/40 km` 搜索及 sample-budget causal A/B。首个 12 帧 stock matrix 因未缩放 `localWeatherRepeat=[100,100]` 被降级为 `UNSCALED_STOCK_WEATHER_CONTROL`。clean-HEAD Stage A1 随后完成 `[100/S,100/S]` health control，并从实际 runtime shader/uniform 验证原生单一 `rayDistance * 1e-5` 与 installed shader/build/patch SHA-256；三档都恢复了 stock weather signal，但均未达到轨道视觉门，`S=120` 仍缺少 limb elevation、柔和 opacity depth 与可读 BSM modulation。当前 checkpoint 为 `SCALED_STOCK_WEATHER_CONTROL_FAIL_MIP_UNPROVEN`、`stockPassingScales=[]`。Stage B/C、V3 分类、mip patch、sample-budget A/B、Task 0P 与原 Task 0–8 均未运行并继续锁定；Conditional Task M 只在用户另行授权时执行同帧只读诊断。证据见 `docs/lubirth-planetary-cloud-evidence/2026-08-12/takram-cloud-scale/stage-a1-scaled-weather/`。
> 计划日期：2026-08-05。
