...
test("emits a bounded CPU cloud-truth asset aligned with the V3 PNG field", async () => {
...
test("Phase -1 kill-spike is query-only and uses the real V3 CPU truth contract", () => {
  ...
  expect(componentSource).toContain(
    "/assets/lubirth/textures/earth-cloud-field-nasa-lite-v3-cpu-truth.bin"
  );
  ...
  expect(componentSource).toContain(
    "densityProfileVersion: \"hierarchical-quadratic-quartic-v1\""
  );
...
test("Phase -1.2 local-density candidate remains V3-derived and sample-bounded", () => {
  ...
  expect(componentSource).toContain("dot(samplePoint - vCenter, vFieldEast)");
  expect(componentSource).toContain("dot(samplePoint - vCenter, vFieldNorth)");
